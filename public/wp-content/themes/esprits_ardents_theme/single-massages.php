...
?>
<main class="mx-auto max-w-xs md:max-w-lg lg:max-w-5xl xl:max-w-6xl lg:mb-20">
...
<nav aria-label="Breadcrumb" class="my-4 flex">
  <ol class="flex overflow-hidden rounded-lg border border-gray-200 text-gray-600">
    <li class="flex items-center">
      <a
        href="/esprits_ardents_v2/public/nos-massages"
        class="flex h-10 items-center gap-1.5 bg-gray-100 px-4 transition hover:text-gray-900"
      >
        <span class="ms-1.5 text-sm font-medium">Massages</span>
      </a>
    </li>
...
<span class="my-4 flex items-center">
  <span class="pr-6 text-2xl font-extrabold font-sans">Autres massages</span>
  <span class="h-px flex-1 bg-black"></span>
</span>
        ...
        if (!empty($posts)) {
            foreach ($posts as $post) { ?>
                ...
                <a href="<?php echo $post-> guid;?>" class="no-underline">
                    <?php
                    $image_info = get_field("image_massage", $post->ID); // Get the image of the current post
                    if ($image_info != null) { ?>
                        <picture>
                            <source media="(min-width: 800px)" srcset="<?php echo $image_info['sizes']["large"]; ?>">
                            <source media="(min-width: 601px)" srcset="<?php echo $image_info['sizes']["medium"]; ?>">
